...
import {
  ...
  ClassRenderer, ClassRendererItem, Field, FieldType, Tile
} from "../dist";
...
window.load = async () => {
  ...
  //设置渲染方式——分级渲染
  layer.renderer = new ClassRenderer();
  layer.renderer.field = field;
  layer.renderer.items.push(new ClassRendererItem(0, 3, symbol2));
  layer.renderer.items.push(new ClassRendererItem(4, 6, symbol3));
  layer.renderer.items.push(new ClassRendererItem(7, 10, symbol4));
  //生成随机数据
  for (let i = 1; i <= 200; i++) {
